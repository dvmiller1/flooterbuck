...
sub stockquote::scan(&$$) {
    ...
    if ( $message =~
/^(?:quote|stock price|index)(?: of| for)? (\^?[A-Z.0-9-]{1,8})\?*\s*$/i
      )
    {
        if ($no_quote) {
            &main::status(
                "Sorry, quote requires LWP and couldn't find it");
            return undef;
        }
        $SIG{CHLD} = "IGNORE";
        ...
        if ( $message =~ /index/ ) {
            ...
            if ( $symbol =~ s/^notfound:// ) {
                $callback->(
                    "I don't know that index name!  I know $symbol");
                if ( defined($pid) ) {
                    exit 0 if ($no_posix);
                    POSIX::_exit(0);
                }
