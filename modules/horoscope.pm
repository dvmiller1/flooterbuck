...
sub horoscope_get($$) {
    if ($no_horoscope) {
        &main::status(
            "Sorry, horoscope.pm requires LWP and can't find it.");
        return "";
    }
    my ( $line, $callback ) = @_;
    $SIG{CHLD} = "IGNORE";
...
sub horoscope_getdata($) {
    my @signs =
      qw(aries taurus gemini cancer leo virgo libra scorpio sagittarius capricorn aquarius pisces);
    ...
    if ($sign_good) {
        ...
        ($horoscope) = ( $string =~
              m|.+<daily_horoscope>(.+)</daily_horoscope>.+|sgi );
...
sub horoscope::scan(&$$) {
    ...
    if (         ( $message =~ /(?:daily )?horoscope(?: for)? (.+)\\?/ )
              or ( $message =~ /(.+)\s+horoscope\??/ ) )
          {
