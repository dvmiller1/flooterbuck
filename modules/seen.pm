...
sub get_timediff($) {
    ...
    $shorttstring =
      sprintf( "%02d", ( $howlong % 60 ) ) . ":$shorttstring";
    if ( $howlong % 60 > 0 ) {
        unshift @tstring,
            ( $howlong % 60 )
          . " minute"
...
sub seen::scan(&$$) {
    ...
    if ( $message =~ /^seen (\S+)/i ) {
        ...
        if ($seen) {
            ...
            $callback->(
                    "$person was last seen on $where $tstring ago, "
                  . "saying: $what "
                  . POSIX::strftime( '[%c]', localtime($when) ) );
            return 1;
        }
        ...
        foreach (@seen) {
            ...
            if ( $nick =~ /$person/i )
            {    # do another regexp match here, because
        ...
        if ( $showmatches > 10 ) {
            $callback->(
"I haven't seen '$person', $who.  There are too many close matches to list."
            );
        } elsif ($showmatches) {
            $callback->(
                "I haven't seen '$person', $who.  $closematches");
        } else {
