...
BEGIN {
    ...
    if ( !$no_slnk ) {
        $user_agent =
          LWP::UserAgent->new( agent => $::version, timeout => 15 );
    }
...
sub slnk_create(@) {
    my %request_ref = @_;
    my ( $callback, $who ) =
      ( $request_ref{callback}, $request_ref{who} );

    my $response = $user_agent->post(
        'http://slnk.org/interface/simplebot/interface',
        { url => $request_ref{url}, automated => "no" }
    );
    ...
    if ( $response->is_success ) {
        ...
        if ( $response_data{STATUS} eq "OK" ) {
            ...
            if ( defined( $response_data{LONG_URL} ) ) {
                $callback->(
                    sprintf( "$who, that short link points to %s",
                        $response_data{LONG_URL} )
                );
            } elsif ( $response_data{SHORT_URL} ) {
                $callback->(
                    sprintf( "$who, your short link is %s",
                        $response_data{SHORT_URL} )
                );
            } else {
                $callback->(
"Sorry $who, slnk.org returned something I didn't understand."
                );
            }
            ...
            # undefined error
            $callback->(
                "$who, It looks like slnk.org is having problems.");
        }
    } else {
        $callback->(
"Sorry $who, I had problems trying to talk to slnk.org. Try again later?"
        );
    }
}
...
sub slnk_getdata(@) {
    ...
    if ( $line =~
/^\s*(?:un)?(?:slnk|xev|fcol)\s+(that(,?\s+please)?|please)?\s*$/i
      )
    {
        return slnk_create(
            url      => &::lastURL( &::channel() ),
    ...
    {
        return slnk_create( url => $1, callback => $callback,
            who => $who );
    }
}
...
sub slnk::get {
    ...
    if ( scalar @unavailable_modules ) {
        my $message = sprintf(
"Sorry $who, slnk.pm requires the following module(s) that were not found: %s",
...
sub slnk::scan(&$$) {
    ...
    if ( $message =~ /^\s*(?:un)?(?:slnk|fcol|xev)\s+(\w+:\S+)\??/i ) {
        ...
        $message =~ /^\s*(?:un)?(?:slnk|fcol|xev)\s+(?:that|please)/i )
    {
