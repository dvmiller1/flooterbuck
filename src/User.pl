...
sub parseUserfile {
    ...
    open( FH, $file )
      or die
"Could not open $file ($!). Perhaps you forgot to rename the example file in conf/infobot.users-dist?\n";
        ...
        if ( !/^#/ && defined $_ ) {
            if (/^UserEntry\s+(.+?)\s/) {
                ...
                if (/\s*\{\s*/) {
                    ...
                    status(
"parse error: User Entry $workname without right brace"
                    );
                }
            }
        }
...
sub verifyUser {
    ...
    &postInc( 'topten', lc $who )
      ;    # add one to the counter of what this user has said
    ...
    if ($VerifWho) {
        ...
        if ( exists $seenVerif{$VerifWho}
            and ( time() - $seenVerif{$VerifWho} > 360 ) )
        {
...
sub userProcessing {
    ...
    if ($VerifWho) {
        if ( $msgType =~ /private/ ) {
            my $unverified_message =
"you must identify yourself; /msg $param{nick} <pass> <command>";
