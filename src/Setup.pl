...
sub setup {
    ...
    %dbs = (
        "is"  => "$param{basedir}/$param{dbname}-is",
    ...
    if ( $param{sanePrefix} ) {
        ...
        if ( !open IGNORE,
            "$param{'confdir'}/$param{sanePrefix}-ignore.txt" )
        {
            &status(
"No fallback ignore file $param{'confdir'}/$param{sanePrefix}-ignore.txt"
            );
        } else {
            while (<IGNORE>) {
                s/^\s+//;
                ...
                /\S/ && do {
                    ...
                    if ( $param{'VERBOSITY'} > 0 ) {
                        &status(
                            "Adding $_ to ignore list (from sane).");
                    }
                };
            }
            close IGNORE;
    ...
    &status(
        "setup: $factoidCount factoids; $isCount IS; $areCount ARE");
}
